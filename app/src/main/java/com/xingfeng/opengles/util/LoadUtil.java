...
public class LoadUtil {
    ...
    public static LoadedObjectVertexNormalAverage2 loadFromFileVertexOnlyAverage2(String fname, Resources r, View mv) {
        //加载后物体的引用
        LoadedObjectVertexNormalAverage2 lo = null;
        ...
        try {
            ...
            //创建3D物体对象
            lo = new LoadedObjectVertexNormalAverage2(mv, vXYZ, nXYZ);
        } catch (Exception e) {
    ...
    public static LoadedObjectVertexNormalFace2 loadFromFileVertexOnlyFace2(String fname, Resources r, View mv) {
        //加载后3D对象的引用
        LoadedObjectVertexNormalFace2 lo = null;
        ...
        try {
            ...
            //创建3D对象
            lo = new LoadedObjectVertexNormalFace2(mv, vXYZ, nXYZ);
        } catch (Exception e) {
            Log.d("load error", "load error");
            e.printStackTrace();
        }
    ...
    //从obj文件中加载携带顶点信息的物体，并自动计算每个顶点的平均法向量
    public static LoadedObjectVertexNormalAverage loadFromFileVertexOnlyAverage(String fname, Resources r, View mv) {
        //加载后物体的引用
    ...
    //首先加载顶点信息，再根据顶点组成三角形面的情况自动计算出每个面的法向量
    //然后将这个面的法向量分配给这个面上的顶点
    public static LoadedObjectVertexNormalFace loadFromFileVertexOnlyFace(String fname, Resources r, View mv) {
        //加载后3D对象的引用
    ...
    //从obj文件中加载携带顶点信息的物体，并自动计算每个顶点的平均法向量
    @SuppressLint("UseSparseArrays")
    public static LoadedObjectVertexNormalTexture2 loadFromFile2
    (String fname, Resources r,View mv)
    {
        //加载后物体的引用
        LoadedObjectVertexNormalTexture2 lo=null;
        //原始顶点坐标列表--直接从obj文件中加载
        ArrayList<Float> alv=new ArrayList<Float>();
        //顶点组装面索引列表--根据面的信息从文件中加载
        ArrayList<Integer> alFaceIndex=new ArrayList<Integer>();
        ...
        try
        {
            ...
            //创建3D物体对象
            lo=new LoadedObjectVertexNormalTexture2(mv,vXYZ,nXYZ,tST);
        }
        catch(Exception e)
        {
            Log.d("load error", "load error");
    ...
    //从obj文件中加载携带顶点信息的物体，并自动计算每个顶点的平均法向量
    @SuppressLint("UseSparseArrays")
    public static LoadedObjectVertexNormalTexture3 loadFromFile3
    (String fname, Resources r,View mv)
    {
        //加载后物体的引用
        LoadedObjectVertexNormalTexture3 lo=null;
        //原始顶点坐标列表--直接从obj文件中加载
        ArrayList<Float> alv=new ArrayList<Float>();
        //顶点组装面索引列表--根据面的信息从文件中加载
        ArrayList<Integer> alFaceIndex=new ArrayList<Integer>();
        ...
        try
        {
            ...
            //创建3D物体对象
            lo=new LoadedObjectVertexNormalTexture3(mv,vXYZ,nXYZ,tST);
        }
        catch(Exception e)
        {
            Log.d("load error", "load error");
    ...
    //从obj文件中加载携带顶点信息的物体，并自动计算每个顶点的平均法向量
    @SuppressLint("UseSparseArrays")
    public static LoadedObjectVertexNormalTexture5 loadFromFile5
    (String fname, Resources r,View mv)
    {
        //加载后物体的引用
        LoadedObjectVertexNormalTexture5 lo=null;
        //原始顶点坐标列表--直接从obj文件中加载
        ArrayList<Float> alv=new ArrayList<Float>();
        //顶点组装面索引列表--根据面的信息从文件中加载
        ArrayList<Integer> alFaceIndex=new ArrayList<Integer>();
        ...
        try
        {
            ...
            //创建3D物体对象
            lo=new LoadedObjectVertexNormalTexture5(mv,vXYZ,nXYZ,tST);
        }
        catch(Exception e)
        {
            Log.d("load error", "load error");
    ...
    //从obj文件中加载携带顶点信息的物体，并自动计算每个顶点的平均法向量
    public static float[][] loadFromFileVertexOnly(String fname,View mv)
    {
        ...
        try
        {
            ...
            while((temps=br.readLine())!=null)
            {
                ...
                {
                    alt.add(Float.parseFloat(tempsa[1])*Constant.MAX_S_GHXP);
                    alt.add(Float.parseFloat(tempsa[2])*Constant.MAX_T_GHXP);
                }
                else if(tempsa[0].trim().equals("f"))
                {
                    int[] index=new int[3];//三个顶点索引值的数组
    ...
    //从obj文件中加载携带顶点信息的物体，并自动计算每个顶点的平均法向量
    @SuppressLint("UseSparseArrays")
    public static LoadedObjectVertexNormalTexture6 loadFromFile6
    (String fname, Resources r,View mv)
    {
        //加载后物体的引用
        LoadedObjectVertexNormalTexture6 lo=null;
        //原始顶点坐标列表--直接从obj文件中加载
        ArrayList<Float> alv=new ArrayList<Float>();
        //顶点组装面索引列表--根据面的信息从文件中加载
        ArrayList<Integer> alFaceIndex=new ArrayList<Integer>();
        ...
        try
        {
            ...
            //创建3D物体对象
            lo=new LoadedObjectVertexNormalTexture6(mv,vXYZ,nXYZ,tST);
        }
        catch(Exception e)
        {
            Log.d("load error", "load error");
    ...
    //从obj文件中加载携带顶点信息的物体，并自动计算每个顶点的平均法向量
    @SuppressLint("UseSparseArrays")
    public static LoadedObjectVertexNormalTexture7 loadFromFile7
    (String fname, Resources r,View mv)
    {
        //加载后物体的引用
        LoadedObjectVertexNormalTexture7 lo=null;
        //原始顶点坐标列表--直接从obj文件中加载
        ArrayList<Float> alv=new ArrayList<Float>();
        //顶点组装面索引列表--根据面的信息从文件中加载
        ArrayList<Integer> alFaceIndex=new ArrayList<Integer>();
        ...
        try
        {
            ...
            //创建3D物体对象
            lo=new LoadedObjectVertexNormalTexture7(mv,vXYZ,nXYZ,tST);
        }
        catch(Exception e)
        {
            Log.d("load error", "load error");
    ...
    public static LoadedObjectVertexNormalTexture9 loadFromFile9(String fname, Resources r,View mv,int texId){
        //加载后物体的引用
        LoadedObjectVertexNormalTexture9 lo=null;
        //原始顶点坐标列表--直接从obj文件中加载
        ArrayList<Float> alv=new ArrayList<Float>();
        //顶点组装面索引列表--根据面的信息从文件中加载
        ArrayList<Integer> alFaceIndex=new ArrayList<Integer>();
        ...
        try
        {
            ...
            //创建3D物体对象
            lo=new LoadedObjectVertexNormalTexture9(mv,vXYZ,nXYZ,tST,texId);
        }
        catch(Exception e)
        {
            Log.d("load error", "load error");
    ...
    public static LoadedObjectVertexNormalTexture10 loadFromFile10
            (String fname, Resources r,View mv) {
        //加载后物体的引用
        LoadedObjectVertexNormalTexture10 lo=null;
        //原始顶点坐标列表--直接从obj文件中加载
        ArrayList<Float> alv=new ArrayList<Float>();
        //顶点组装面索引列表--根据面的信息从文件中加载
        ArrayList<Integer> alFaceIndex=new ArrayList<Integer>();
        ...
        try
        {
            ...
            //创建3D物体对象
            lo=new LoadedObjectVertexNormalTexture10(mv,vXYZ,nXYZ,tST);
        }
        catch(Exception e)
        {
            Log.d("load error", "load error");
