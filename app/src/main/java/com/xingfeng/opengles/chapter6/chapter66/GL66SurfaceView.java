...
class GL66SurfaceView extends GLSurfaceView
{
    private final float TOUCH_SCALE_FACTOR = 180.0f/320;//角度缩放比例
    private SceneRenderer mRenderer;//场景渲染器
    DirectionBall ball;//球

    private float mPreviousY;//上次的触控位置Y坐标
    private float mPreviousX;//上次的触控位置X坐标
    private float lightOffset = 2;

    public GL66SurfaceView(Context context) {
        super(context);
    ...
    private class SceneRenderer implements Renderer
    {
        public void onDrawFrame(GL10 gl)
        {
            ...
            //绘制球
            MatrixState.pushMatrix();
            MatrixState.translate(-1.f, 0, 0);//沿x轴正方向平移
            ball.drawSelf();
            MatrixState.popMatrix();

            //绘制球
            MatrixState.pushMatrix();
            MatrixState.translate(1f, 0, 0);//沿x轴正方向平移
            ball.drawSelf();
            MatrixState.popMatrix();
        ...
        public void onSurfaceCreated(GL10 gl, EGLConfig config) {
            ...
            //创建球对象
            ball=new DirectionBall(GL66SurfaceView.this);
            //打开深度检测
