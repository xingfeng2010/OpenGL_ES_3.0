...
public class FlagRect
{
    ...
    public FlagRect(View mv)
    {
        //初始化顶点坐标与着色数据
        initVertexData();
        //初始化shader
        initShader(mv,0,"chapter202/chapter202.1/vertex_tex_x.glsl");
        initShader(mv,1,"chapter202/chapter202.1/vertex_tex_xie.glsl");
        initShader(mv,2,"chapter202/chapter202.1/vertex_tex_xy.glsl");
        //启动一个线程定时换帧
        new Thread()
        {
            public void run()
    ...
    public void initShader(View mv,int index,String vertexName)
    {
        ...
        //加载片元着色器的脚本内容
        String mFragmentShader=ShaderUtil.loadFromAssetsFile("chapter202/chapter202.1/frag_tex.glsl", mv.getResources());
        //基于顶点着色器与片元着色器创建程序
    ...
    public void drawSelf(int texId)
    {
        ...
        //将本帧起始角度传入shader程序
        GLES30.glUniform1f(maStartAngleHandle[currIndex], currStartAngle);
        //将横向长度总跨度传入shader程序
        GLES30.glUniform1f(muWidthSpanHandle[currIndex], WIDTH_SPAN);
        //将顶点位置数据传入渲染管线
